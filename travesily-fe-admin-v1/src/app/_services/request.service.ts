...
export class RequestService {
  ...
  denyHotel(id){
    ...
    return this.http.patch(`${this.baseUrl}/deny-request`, undefined, {params: params})
  }

  acceptHotel(id: string){
    const params = new HttpParams().append('requestId', id)
    return this.http.patch(`${this.baseUrl}/accept-request`,{params: params})
  }

  getPageRequest(status: number,page: number, pageSize: number){
    const params = new HttpParams().append('page', page).append('pageSize', pageSize).append('status', status)
