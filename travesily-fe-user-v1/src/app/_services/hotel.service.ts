...
export class HotelService {
  ...
  searchHotel(destination: number, start: Date, end: Date, adultNumber: number, roomNumber: number, page: number, pageSize: number) {
    ...
    const params = new HttpParams().append('districtId', destination).append('dateIn', dateIn)
      .append('dateOut', dateOut).append('numberOfPeople', adultNumber).append('numberOfRoom', roomNumber)
      .append('page', page).append('pageSize', pageSize);
